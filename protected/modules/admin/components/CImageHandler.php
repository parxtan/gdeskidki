...
class CImageHandler extends CApplicationComponent
{
    ...
    public function resize($toWidth, $toHeight, $proportional = true)
    {
		...
		$toWidth = $toWidth !== false ? min($toWidth, $this->width) : $this->width;
		$toHeight = $toHeight !== false ? min($toHeight, $this->height) : $this->height;
	...
	public function adaptiveThumb($width, $height, $startX = false, $startY = false)
	{
        $this->checkLoaded();
		...
		$this->crop($width, $height, $startX, $startY);
