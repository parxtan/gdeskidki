...
class Users extends CActiveRecord
{
	...
	{
		return array(
			array('email, name, lastname, password', 'required', 'message'=>'Заполните поле'),
			array('password, password2, email', 'length', 'max'=>128),
			array('name, lastname, city, status', 'length', 'max'=>50),		
			array('email','email','message'=>'{attribute} некорректен'),
			array('password','compareWithPassword','on'=>'update','message'=>'Не совпадает с текущим паролем'),
	...
	public function compareWithPassword($attribute,$params)
	{
		...
		if($user->password != md5($this->$attribute))
			$this->addError($attribute,'Не совпадает с текущим паролем');
	}
}